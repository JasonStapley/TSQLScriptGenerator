                                    ...
                                    for (int i = 0; i < reader.FieldCount; i++)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                else if (datadatabase.Rows[i]["DataTypeName"].Equals("money") == true)
                                                {
                                                    
                                                    tr.Write(((decimal)reader[i]).ToString());
                                                }
                                                else
                                                {
                                                    tr.Write(reader[i]);
...
/// <returns></returns>
        ...
        public static string RenderFieldValue(int i, SqlDataReader reader, DataTable database)
        {
            ...
            else
            {
                ...
                {
                    string datatype = database.Rows[i]["DataTypeName"].ToString();
                    ...
                    else if (database.Rows[i]["DataTypeName"].Equals("money") == true)
                    {
                        return ((decimal)reader[i]).ToString();
                    }
                    else
                    {
        ...
        public static void ExportCSharpCall(string sqlserver, string sqldatabase, string file, string query, string table, string timeout)
        {
            ...
            using (FileStream fs = new FileStream(file, FileMode.CreateNew, FileAccess.Write, FileShare.ReadWrite))
            {
                using (TextWriter tr = new StreamWriter(fs))
                {
                    ...
                    tr.WriteLine(" SqlConn.ChangeDatabase(\"" + sqldatabase + "\");");

                    tr.WriteLine(" using (SqlCommand SqlCmd = new SqlCommand(\"" + query.Replace("\n"," ") + "\", SqlConn)) ");
                    tr.WriteLine("      {");
                    ...
                    if (reader.HasRows == true)
                    {
                        ...
                        using (FileStream fs = new FileStream(file, FileMode.CreateNew, FileAccess.Write, FileShare.ReadWrite))
                        {
                            ...
                            {                                
                                ...
                                tr.WriteLine("SET @TableCursor = CURSOR READ_ONLY FOR " + query);
